...
import {Body, Container, Frame, Header, Inner, Item, Title} from './styles/accordian';
...
Accordian.Item = function AccordianItem({children, ...restProps}) {
    ...
    return <ToggleContext.Provider
        value={{toggle, setToggle}}><Item {...restProps}>{children}</Item></ToggleContext.Provider>
}

Accordian.Header = function AccordianHeader({children, ...restProps}) {
    const {toggle, setToggle} = useContext(ToggleContext);
    return <Header onClick={() => setToggle((toggle) => !toggle)} {...restProps}>{children}
        {toggle ? (
